...
def write_meter_rule(p4info_helper, ingress_sw, meter_name, cir, cburst, pir, pburst):
    ...
    :param pir: bits/s
    """
    for i in range(0, 1024): # meter array every index
        meter_entry = p4info_helper.buildMeterEntry(
            meter_name="MyIngress.my_meter",
            cir=cir, cburst=cburst,
            pir=pir, pburst=pburst,
            index=i
            )
        ingress_sw.WriteMeterEntry(meter_entry)
    print("Installed meter Rule on %s" % ingress_sw.name)
...
def write_m_filter_rule(p4info_helper, ingress_sw, meter_tag, action_name):
    ...
    ingress_sw.WriteTableEntry(table_entry)
    print("Installed m_filter forward Rule on %s" % ingress_sw.name)
