...
class P4InfoHelper(object):
    ...
    def get(self, entity_type, name=None, id=None):
        """
        先根据`entity_type`找出p4Info中所有该类型的一级实体，
        然后对这些实体根据preamble的`name`或 `id`进行匹配，得到唯一匹配的实体
        """
    ...
    def __getattr__(self, attr):
        ...
        # 把 `get_XXX_name`、`get_XXX_id`调用的`XXX`提取出来，作为
        # entity_type传入`get_name(self, entity_type, id)`、`get_id(self, entity_type, name) `方法
    ...
    def get_match_field_pb(self, table_name, match_field_name, value):
        ...
        if match_type == p4info_pb2.MatchField.EXACT:
            ...
            # 引用类型 p4runtime_match.exact.value = encode(value, bitwidth)
    ...
    def buildTableEntry(self,
        ...
        # get_tables_id会调用 getattr
        # 最终等价于 self.get_id("tables", table_name)
